...
export const ResultsDashboard: React.FC<Props> = ({ state, error, onRecheck, onUpdateRisk }) => {
  ...
  if (error) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-studio-900/80 rounded-2xl border border-studio-danger/50 p-8 backdrop-blur-sm shadow-2xl">
        <div className="bg-studio-danger/10 p-4 rounded-full mb-4 shadow-[0_0_30px_rgba(255,51,102,0.3)] border border-studio-danger/30">
        ...
        <p className="text-sm text-gray-300 text-center max-w-lg mb-6 font-mono">{error}</p>
        <div className="bg-studio-950 p-5 rounded-xl border border-studio-700 w-full max-w-lg shadow-inner">
          ...
          <ul className="text-xs text-gray-300 list-disc list-inside space-y-2 font-mono">
  ...
  if (!hasData && !isCoreRunning) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-gray-400 bg-studio-900/60 rounded-2xl border border-studio-700 border-dashed backdrop-blur-sm shadow-xl">
        <div className="hud-corners p-8 flex flex-col items-center">
          <Clapperboard className="w-12 h-12 text-studio-600 mb-4" />
          <h3 className="text-lg font-bold text-gray-200 mb-2 font-mono uppercase tracking-widest">Standby for Input</h3>
          <p className="text-xs max-w-md text-center text-gray-400 font-mono">Upload a screenplay to initialize the ADK pipeline and generate the production matrix.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-studio-900 rounded-2xl border border-studio-700 overflow-hidden shadow-2xl relative">
      
      {/* Top Summary Bar (HUD Style) */}
      {hasData && (
        <div className="grid grid-cols-4 divide-x divide-studio-700 border-b border-studio-700 bg-studio-950">
          <div className="p-4 flex items-center gap-4 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-0.5 bg-studio-accent/70"></div>
            <div className="bg-studio-accent/10 p-2.5 rounded-lg border border-studio-accent/30"><FileText className="w-5 h-5 text-studio-accent" /></div>
            <div>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold font-mono">Total Scenes</p>
              <p className="text-2xl font-bold text-white font-mono">{totalScenes}</p>
            </div>
          </div>
          <div className="p-4 flex items-center gap-4 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-0.5 bg-studio-success/70"></div>
            <div className="bg-studio-success/10 p-2.5 rounded-lg border border-studio-success/30"><Calendar className="w-5 h-5 text-studio-success" /></div>
            <div>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold font-mono">Shoot Days</p>
              <p className="text-2xl font-bold text-white font-mono">{totalDays || '-'}</p>
            </div>
          </div>
          <div className="p-4 flex items-center gap-4 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-0.5 bg-studio-gold/70"></div>
            <div className="bg-studio-gold/10 p-2.5 rounded-lg border border-studio-gold/30"><MapPin className="w-5 h-5 text-studio-gold" /></div>
            <div>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold font-mono">Locations</p>
              <p className="text-2xl font-bold text-white font-mono">{totalLocations}</p>
            </div>
          </div>
          <div className="p-4 flex items-center gap-4 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-0.5 bg-studio-danger/70"></div>
            <div className="bg-studio-danger/10 p-2.5 rounded-lg border border-studio-danger/30"><AlertTriangle className="w-5 h-5 text-studio-danger" /></div>
            <div>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold font-mono">Open Risks</p>
      ...
      {/* Tabs Header */}
      <div className="flex border-b border-studio-700 bg-studio-900 px-4 pt-3 gap-2">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center px-4 py-2.5 text-xs font-bold uppercase tracking-widest transition-all rounded-t-lg border-t border-x mb-[-1px] font-mono
                ${isActive ? 'text-studio-accent border-studio-700 bg-studio-950 shadow-[0_-4px_10px_rgba(0,229,255,0.05)]' : 'text-gray-400 border-transparent hover:text-gray-200 hover:bg-studio-800'}
      ...
      {/* Content Area */}
      <div className="flex-1 overflow-y-auto p-6 bg-studio-950 custom-scrollbar relative">
        ...
        {isCoreRunning && activeAgentKey && (
          <div className="absolute inset-0 bg-studio-950/90 backdrop-blur-md z-20 flex flex-col items-center justify-center p-8">
            <div className="hud-corners p-10 bg-studio-900 border border-studio-700 shadow-2xl w-full max-w-lg">
                ...
                {Object.entries(agentDisplayNames).map(([key, name]) => {
                  ...
                  return (
                    <div key={key} className={`flex items-center justify-between p-3 rounded border font-mono text-xs uppercase tracking-widest ${
                      ...
                      'bg-studio-950 border-studio-800 text-gray-500'
                    }`}>
                      <span>{name}</span>
        ...
        {activeTab === 'overview' && hasData && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="grid grid-cols-2 gap-6">
              {/* Schedule Snapshot */}
              <div className="bg-studio-900 border border-studio-700 rounded-xl p-6 shadow-lg relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-studio-success/50"></div>
                <h3 className="text-xs font-bold text-gray-300 uppercase tracking-widest mb-5 flex items-center font-mono"><Calendar className="w-4 h-4 mr-2 text-studio-success"/> Schedule Snapshot</h3>
                {state.productionPlanner.data ? (
                  <div className="space-y-3">
                    {state.productionPlanner.data.slice(0, 3).map(day => (
                      <div key={day.dayNumber} className="flex items-center justify-between p-3 bg-studio-950 rounded border border-studio-800">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="text-sm text-gray-100 truncate max-w-[200px] font-medium">{day.locations.join(', ')}</div>
                        </div>
                        <div className="text-xs text-gray-400 font-mono">{day.scenes.length} SCENES</div>
                      </div>
                    ))}
                    {state.productionPlanner.data.length > 3 && (
                      <button onClick={() => setActiveTab('plan')} className="w-full py-3 mt-2 text-xs text-studio-accent hover:text-white font-bold uppercase tracking-widest transition-colors border border-studio-700 rounded bg-studio-950 hover:bg-studio-800">View Full Schedule</button>
                    )}
                  </div>
                ) : (
                  <div className="text-xs text-gray-500 font-mono uppercase tracking-widest">Generation pending...</div>
                )}
              </div>
              ...
              <div className="bg-studio-900 border border-studio-700 rounded-xl p-6 shadow-lg relative overflow-hidden">
                ...
                <h3 className="text-xs font-bold text-gray-300 uppercase tracking-widest mb-5 flex items-center font-mono"><ShieldAlert className="w-4 h-4 mr-2 text-studio-danger"/> Critical Risks</h3>
                {state.riskAnalyst.data ? (
                  <div className="space-y-3">
                    {state.riskAnalyst.data.filter(r => r.severity === 'High' && r.status !== 'Resolved').slice(0, 3).map((risk, i) => (
                      <div key={i} className="flex items-start gap-3 p-3 bg-studio-950 rounded border border-studio-danger/30">
                        <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0 text-studio-danger" />
                        <div>
                          <p className="text-sm text-gray-100 font-medium line-clamp-1">{risk.description}</p>
                          <p className="text-xs text-gray-400 mt-1 line-clamp-1">{risk.mitigation}</p>
                        </div>
                      </div>
                    ))}
                    {state.riskAnalyst.data.length > 3 && (
                      <button onClick={() => setActiveTab('risks')} className="w-full py-3 mt-2 text-xs text-studio-accent hover:text-white font-bold uppercase tracking-widest transition-colors border border-studio-700 rounded bg-studio-950 hover:bg-studio-800">View Risk Matrix</button>
                    )}
                  </div>
                ) : (
                  <div className="text-xs text-gray-500 font-mono uppercase tracking-widest">Analysis pending...</div>
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'script' && state.scriptAnalyst.data && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-500">
            {state.scriptAnalyst.data.map((scene, idx) => (
              <div key={idx} className="bg-studio-900 p-6 rounded-xl border border-studio-700 shadow-lg relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest bg-studio-950 px-2.5 py-1 rounded text-gray-400 border border-studio-700 font-mono">{scene.setting} • {scene.timeOfDay}</span>
                </div>
                <p className="text-sm text-gray-200 mb-5 leading-relaxed font-script bg-studio-950 p-3 rounded border border-studio-800">{scene.summary}</p>
                
                <div className="space-y-4">
                  {scene.characters.length > 0 && (
                    <div>
                      <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold block mb-2 flex items-center gap-1.5 font-mono"><Users className="w-3.5 h-3.5"/> Characters</span>
                      <div className="flex flex-wrap gap-2">
                        {scene.characters.map(c => <span key={c} className="bg-studio-950 border border-studio-700 px-2.5 py-1 rounded text-xs text-gray-200 font-medium">{c}</span>)}
                      </div>
                    </div>
                  )}
                  {(scene.props.length > 0 || scene.specialRequirements.length > 0) && (
                    <div>
                      <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold block mb-2 font-mono">Props & Reqs</span>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {scene.dependencies && scene.dependencies.length > 0 && (
                    <div className="bg-studio-accent/5 p-3 rounded border border-studio-accent/20 mt-3">
                      ...
                      <ul className="list-disc list-inside text-xs text-gray-200 space-y-1.5 font-mono">
            ...
            {state.researchAgent.data && state.researchAgent.data.length > 0 ? (
              <div className="grid grid-cols-1 gap-6">
                {state.researchAgent.data.map((item, idx) => (
                  <div key={idx} className="bg-studio-900 p-6 rounded-xl border border-studio-700 shadow-lg flex flex-col md:flex-row gap-8 relative overflow-hidden">
                    ...
                    <div className="md:w-1/3 shrink-0">
                      ...
                      <div className="bg-studio-950 p-3 rounded border border-studio-800 mb-4">
                        <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-1.5 font-bold">Search Query</p>
                    ...
                    <div className="md:w-2/3 bg-studio-950 p-5 rounded-lg border border-studio-800 flex flex-col">
                      <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-3 flex items-center font-mono"><Search className="w-3.5 h-3.5 mr-2 text-studio-accent"/> Verified Findings</p>
                      <p className="text-sm text-gray-100 leading-relaxed mb-5 font-mono">{item.simulatedFindings}</p>
                      
                      {/* Source Attribution */}
                      <div className="mt-auto bg-studio-900 p-4 rounded border border-studio-700">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold flex items-center font-mono">
                        ...
                        {item.excerpt && (
                          <p className="text-xs text-gray-300 italic border-l-2 border-studio-600 pl-3 py-1 font-script">"{item.excerpt}"</p>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center p-16 bg-studio-900 rounded-xl border border-studio-700 border-dashed">
                <CheckCircle2 className="w-16 h-16 text-studio-success mb-5 opacity-50" />
                <h3 className="text-xl font-bold text-gray-100 mb-2 font-mono uppercase tracking-widest">No Logistical Constraints</h3>
                <p className="text-sm text-gray-400 text-center max-w-md font-mono">The Research Agent analyzed the scenes and determined that no external verification was required for this script.</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'plan' && state.productionPlanner.data && (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-in fade-in duration-500">
            {state.productionPlanner.data.map((day, idx) => (
              <div key={idx} className="bg-studio-900 rounded-xl border border-studio-700 shadow-lg overflow-hidden flex flex-col">
                <div className="bg-studio-800 p-5 border-b border-studio-700 flex justify-between items-center">
                  <div className="flex items-center gap-4">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="mb-5">
                    <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold flex items-center mb-2 font-mono"><MapPin className="w-3.5 h-3.5 mr-1.5"/> Locations</span>
                    <p className="text-sm text-gray-100 font-medium leading-relaxed">{day.locations.join(' • ')}</p>
                  </div>
                  <div className="mb-6">
                    <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold flex items-center mb-2 font-mono"><Clapperboard className="w-3.5 h-3.5 mr-1.5"/> Scenes</span>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mt-auto bg-studio-950 p-4 rounded border border-studio-800">
                    <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold block mb-1.5 font-mono">Logistical Notes</span>
                    <p className="text-xs text-gray-300 italic leading-relaxed font-script">{day.notes}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="space-y-5 animate-in fade-in duration-500">
            {state.riskAnalyst.data.length > 0 ? (
              state.riskAnalyst.data.map((risk, idx) => {
                ...
                return (
                  <div key={risk.id || idx} className={`bg-studio-900 p-6 rounded-xl border-l-4 shadow-lg flex flex-col gap-5 transition-all ${
                    isResolved ? 'opacity-50 border-l-gray-600 bg-studio-900/50' : 
                    ...
                    <div className="flex flex-col md:flex-row gap-6 relative z-10">
                      ...
                      {!isResolved && (
                        <div className="md:w-2/3 bg-studio-950 p-5 rounded border border-studio-800 flex flex-col justify-center">
                          <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-2 flex items-center font-mono"><CheckCircle2 className="w-4 h-4 mr-2 text-studio-success"/> Mitigation Strategy</span>
                    ...
                    <div className="mt-2 pt-4 border-t border-studio-800 flex items-center gap-4 relative z-10">
                      <select 
                        ...
                        className="bg-studio-950 border border-studio-700 text-xs rounded px-3 py-2 text-gray-200 focus:outline-none focus:border-studio-accent font-mono uppercase tracking-widest font-bold cursor-pointer"
                      >
                      ...
                      <input 
                        ...
                        className="flex-1 bg-studio-950 border border-studio-700 rounded px-4 py-2 text-xs text-gray-200 focus:border-studio-accent focus:outline-none font-mono"
                      />
                    </div>
                  </div>
                );
              ...
              <div className="flex flex-col items-center justify-center p-16 bg-studio-900 rounded-xl border border-studio-700 border-dashed">
                <CheckCircle2 className="w-16 h-16 text-studio-success mb-5 opacity-50" />
                <h3 className="text-xl font-bold text-gray-100 mb-2 font-mono uppercase tracking-widest">No Significant Risks</h3>
                <p className="text-sm text-gray-400 text-center max-w-md font-mono">The Risk Analyst reviewed the schedule and found no major logistical or safety concerns.</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'changes' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            {/* Header with button */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-studio-900 p-6 rounded-xl border border-studio-700 shadow-lg gap-4">
              <div>
            ...
            {state.changeMonitor.history && state.changeMonitor.history.length > 0 ? (
              <div className="space-y-10 relative pl-10 before:absolute before:inset-y-0 before:left-4 before:w-0.5 before:bg-studio-700">
                {state.changeMonitor.history.map((report, index) => (
                  <div key={report.id} className="relative">
                    ...
                    {/* Content Card */}
                    <div className="bg-studio-900 rounded-xl border border-studio-700 shadow-xl overflow-hidden">
                      <div className="bg-studio-800 p-5 border-b border-studio-700 flex justify-between items-center">
                        ...
                        <span className="text-xs text-gray-400 font-mono bg-studio-950 px-3 py-1.5 rounded border border-studio-700">
                      ...
                      <div className="p-6 space-y-8">
                        ...
                        <div className="bg-studio-danger/10 border border-studio-danger/30 p-5 rounded bg-warning-stripes">
                          ...
                          <p className="text-gray-100 text-sm leading-relaxed font-mono">{report.changeReason}</p>
                        </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-4">
                              {report.changedFacts.map((fact, idx) => (
                                <div key={idx} className="bg-studio-950 rounded border border-studio-700 shadow-md overflow-hidden">
                                  <div className="bg-studio-900 p-4 border-b border-studio-700 flex justify-between items-center">
                                    <span className="font-bold text-gray-100 font-mono uppercase tracking-tight">{fact.topic}</span>
                                    <div className="flex items-center gap-3">
                                      <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold font-mono">Affected:</span>
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-studio-700">
                                    <div className="p-5 bg-studio-950">
                                      <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold flex items-center mb-3 font-mono">
                                    ...
                                    <div className="p-5 bg-studio-danger/10">
                                      ...
                                      <p className="text-sm text-gray-100 font-medium mb-4 font-mono">{fact.newFinding}</p>
                                      
                                      {fact.sourceUrl && (
                                        <div className="bg-studio-900 p-3 rounded border border-studio-danger/20">
                                          <div className="flex items-center justify-between mb-2">
                                            <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold flex items-center font-mono">
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              {report.updatedPlan.map((day, idx) => (
                                <div key={idx} className="bg-studio-950 p-4 rounded border border-studio-700 shadow-md flex justify-between items-center">
                                  <div>
                                    <span className="font-black text-studio-accent text-lg mr-3 font-mono">D{day.dayNumber}</span>
                                    <span className="text-sm text-gray-200 font-medium">{day.locations.join(', ')}</span>
                                  </div>
                                  <div className="flex gap-1.5">
                                    {day.scenes.map(s => <span key={s} className="bg-studio-900 border border-studio-700 text-gray-300 px-2 py-0.5 rounded text-xs font-bold font-mono">{s}</span>)}
                                  </div>
                                </div>
                              ))}
                            </div>
                            ...
                            <div>
                              ...
                              <div className="space-y-3">
                                {report.newOrReopenedRisks.map((risk, idx) => (
                                  <div key={idx} className="bg-studio-950 p-4 rounded border-l-4 border-studio-gold border-y border-r border-studio-700 shadow-md">
                                    ...
                                    <div className="bg-studio-900 p-3 rounded border border-studio-800 mb-2">
                                      <span className="text-[10px] text-gray-400 uppercase tracking-widest font-bold block mb-1.5 font-mono">Mitigation</span>
              ...
              <div className="flex flex-col items-center justify-center p-16 bg-studio-900 rounded-xl border border-studio-700 border-dashed">
                ...
                <h3 className="text-xl font-bold text-gray-200 mb-2 font-mono uppercase tracking-widest">No Disruptions Yet</h3>
                <p className="text-sm text-gray-400 text-center max-w-md font-mono">Click "Simulate Disruption" to see how the Change Monitor adapts the production plan to real-world changes.</p>
              </div>
            )}
          </div>
        )}
